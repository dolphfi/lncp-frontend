...
const AppSidebar = () => {
  ...
  const [displayAvatar, setDisplayAvatar] = React.useState<string | undefined>(
    undefined
  );
  ...
  React.useEffect(() => {
    ...
    return () =>
      document.removeEventListener("profile:updated", handler as EventListener);
  }, [displayName]);
  // Écoute les mises à jour du profil pour rafraîchir instantanément l'UI
  React.useEffect(() => {
    const handler = (e: Event) => {
  ...
  const navGroups = React.useMemo(() => {
    ...
    groups.push({
      title: "Général",
      items: [{ to: "/dashboard", label: "Tableau de bord", icon: Gauge }],
    });
            ...
            ? [
                {
                  ...
                  subItems: [
                    ...(canManageNotes
                      ? [
                          {
                            to: "/academic/notes/entry",
                            label: "Saisie de notes",
                          },
                        ]
                      : []),
                    {
                      to: "/academic/notes/list",
                    ...
                    user?.role !== "CENSORED"
                      ? [
                          {
                            to: "/academic/bulletins/individual",
                            label: "Bulletin Individuel",
                          },
                          {
                            to: "/academic/bulletins/collective",
                            label: "Bulletin Collectif",
                          },
                        ]
                      : []),
                  ],
        ...
        items: [
          {
            ...
            icon: UserRoundCog,
            subItems: [{ to: "/employee", label: "Employés" }],
          },
            ...
            ? [
                {
                  ...
                  icon: Fence,
                  subItems: [{ to: "/roles", label: "Liste" }],
                },
                {
                  to: "/ress",
                  label: "Ressources Financière",
                  icon: Wallet,
                  subItems: [{ to: "/payments", label: "Paiments Élèves" }],
                },
              ]
            : []),
        ],
    ...
    <Sidebar collapsible="icon" variant="sidebar">
      <SidebarHeader
        className={cn(
          ...
          state === "expanded" ? "justify-between px-4" : "justify-center px-2"
        )}
      >
        {state === "expanded" && (
      ...
      <SidebarContent className="space-y-0">
        {navGroups.map((group) => (
          <SidebarGroup key={group.title} className="mb-0 mt-0">
            <SidebarGroupLabel className="px-2 py-0 text-xs font-medium text-muted-foreground mb-0 mt-0">
              {group.title}
            </SidebarGroupLabel>
