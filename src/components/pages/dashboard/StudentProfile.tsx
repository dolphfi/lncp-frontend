...
const StudentProfilePage: React.FC = () => {
  ...
  const averageNote = currentData.notes.length > 0 
    ? (() => {
            ...
            <div className="relative p-5">
              ...
              <div className="text-center">
                <h2 className="text-base font-semibold text-blue-900">
                  {currentData.studentInfo.firstName} {currentData.studentInfo.lastName}
                </h2>
              ...
              <div className="mt-4 space-y-2 text-xs">
                <div className="flex items-center justify-between p-2 rounded-lg bg-white/50 border border-gray-200/80">
                  ...
                  <span className="text-blue-900 font-semibold">{currentData.studentInfo.classroom}</span>
                </div>
                <div className="flex items-center justify-between p-2 rounded-lg bg-white/50 border border-gray-200/80">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="text-blue-900 font-semibold">{currentData.studentInfo.age} ans</span>
                </div>
                <div className="flex items-center justify-between p-2 rounded-lg bg-white/50 border border-gray-200/80">
                  <div className="flex items-center gap-2">
                    <Clock className="w-3.5 h-3.5 text-blue-600" />
                    <span className="text-blue-900/80 font-medium">Vacation</span>
                  </div>
                  <span className="text-blue-900 font-semibold">{currentData.studentInfo.vacation}</span>
                </div>
              </div>
            </div>
          </div>
              ...
              {currentData.paymentRequired ? (
                ...
                <div className="space-y-2">
                  {latestNotes.map((note, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-white/50 border border-gray-200/80">
                      <span className="text-xs text-blue-900/80 truncate">{note.course}</span>
                  ...
                  <div className="flex items-center justify-between p-2 rounded-lg bg-blue-50 border border-blue-200/80 mt-3">
                    <span className="text-xs font-semibold text-blue-900">Moyenne</span>
                    <span className="text-xs font-bold text-blue-900">{averageNote}/100</span>
                  </div>
                </div>
              ) : (
                <div className="text-center py-4">
                  <p className="text-xs text-blue-900/50">Aucune note disponible</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-2xl backdrop-blur-xl bg-white/90 overflow-hidden border border-gray-200">
            <div className="p-4">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-white/50 border border-gray-200/80">
                <div className="flex items-center gap-2">
                  <div className={`p-1.5 rounded-full ${currentData.paymentRequired ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                    {currentData.paymentRequired ? (
                      <XCircle className="w-3.5 h-3.5" />
                    ) : (
                      <CheckCircle className="w-3.5 h-3.5" />
                ...
                <span className={`text-xs font-bold px-3 py-1.5 rounded-full ${currentData.paymentRequired ? 'bg-red-50 text-red-900' : 'bg-green-50 text-green-900'}`}>
            ...
            <div className="p-5">
              ...
              {currentData.attendances.filter(a => a.status !== 'Présent').length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-blue-900/70 font-medium">Aucune absence ou retard</p>
                  <p className="text-xs text-blue-900/50 mt-1">Excellent comportement !</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {currentData.attendances.filter(a => a.status !== 'Présent').slice(0, 5).map((att: Attendance) => (
                    <div 
                      key={att.id} 
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-xs text-blue-900/70">{formatDate(att.timestamp)}</p>
                          </div>
                          <p className="text-xs text-blue-900/60 mt-0.5 truncate">{att.reason}</p>
                        </div>
                      </div>
            ...
            <div className="p-5">
              ...
              {currentData.paymentRequired ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-blue-900/70 font-medium mb-1">Accès restreint</p>
                ...
                <div className="space-y-3">
                    ...
                    {latestNotes.map((note, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-blue-900 truncate">{note.course}</p>
                          </div>
                        </div>
                        <div className="flex-shrink-0 ml-3">
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-300">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-blue-900">Moyenne Générale</span>
                    </div>
                    <span className="text-lg font-bold text-blue-900">{averageNote}/100</span>
                  </div>
                </div>
              ) : (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-blue-900/70 font-medium">Aucune note disponible</p>
                  <p className="text-xs text-blue-900/50 mt-1">Les notes apparaîtront ici</p>
                </div>
              )}
            </div>
          </div>
